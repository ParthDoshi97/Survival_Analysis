...
```{r}
# Distributions of FLCs (on raw and log scales)
p1 <- ggplot(dat_final, aes(kappa)) + geom_histogram(bins = 50,binwidth = 0.1, fill = "coral", color = "black") + theme_minimal() + ggtitle("Kappa")
p2 <- ggplot(dat_final, aes(lambda)) + geom_histogram(bins = 50,binwidth = 0.1, fill = "coral", color = "black") + theme_minimal() + ggtitle("Lambda")
p3 <- ggplot(dat_final, aes(kappa_log)) + geom_histogram(bins = 50,binwidth = 0.1, fill = "coral", color = "black") + theme_minimal() + ggtitle("log(Kappa)")
p4 <- ggplot(dat_final, aes(lambda_log)) + geom_histogram(bins = 50,binwidth = 0.1, fill = "coral", color = "black") + theme_minimal() + ggtitle("log(Lambda)")
p5 <- ggplot(dat_final, aes(flc_ratio_log)) + geom_histogram(bins = 50,binwidth = 0.1, fill = "coral", color = "black") + theme_minimal() + ggtitle("FLC ratio (kappa/lambda)")
...
-   Observational cohort; residual confounding is possible.
-   `chapter` is only defined for decedents; overall missingness is high by design and thus not used as a predictor.
-   Single geographic region and age ≥50; generalizability may be limited.
-   Biomarker assays and calibration choices (e.g., cut‑points in `flc.grp`) affect interpretability across studies.
